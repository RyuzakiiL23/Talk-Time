	...
	const handleSubmit = async (event) => {
		...
		try {
			const res = await fetch("http://localhost:8080/api/auth/signup", {
				...
				body: JSON.stringify(inputs), // Send inputs as JSON string in the request body
			});
			...
			console.log(data);
			dispatch(authentificated()); // assuming you have a Redux action for authentication
		} catch (error) {
			console.log(`error: ${error} `);
		}
	};

	const authentification = useSelector((state) => state.auth.value); // Access the counter state
